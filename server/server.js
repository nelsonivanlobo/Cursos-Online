...
app.use(cors({
    origin: 'http://127.0.0.1:5501', // Cambia esto por el origen de tu frontend
...
app.use(session({
    ...
    cookie: {
        ...
        httpOnly: true,
        sameSite: 'lax', // Asegúrate de que sea compatible con tu frontend
    },
}));
...
const transporter = nodemailer.createTransport({
    ...
    port: 587, // Cambia esto si es necesario
    ...
    try {
        ...
        if (action === 'payment.created') {
            ...
            if (paymentInfo.body.status === 'approved') {
                ...
                const provincia = metadata.provincia || null; // Proporcionado en metadata si aplica
                const idCurso = metadata.curso; // Asegúrate de que este campo esté en metadata
...
app.post('/administrador', (req, res) => {
    ...
    connection.query(query, [nombre, email], (err, results) => {
        ...
        req.session.isAuthenticated = true; // Asegúrate de tener configurado el middleware de sesión
        req.session.adminId = admin.id; // Guardar el ID del administrador si es necesario
